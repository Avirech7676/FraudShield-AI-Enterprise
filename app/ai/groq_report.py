...
class EnterpriseFraudReporter:
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning(
                "GROQ_API_KEY is not set. LLM capabilities will run in simulation mode."
            )
            self.client = None
        else:
            try:
                ...
                logger.exception(f"Groq initialization failed : {e}")
                self.client = None
    ...
    def health(self):
        return {
            ...
            "simulation_mode": self.client is None,
        }
    ...
    def _call_llm(
        self,
        prompt,
        system_prompt="You are a Senior Fraud Investigation Officer.",
    ):
        if not self.client:
    ...
    def explain_flag(
        ...
        shap_summary = "No SHAP explanation available."
        ...
        if top_factors:
            ...
            for factor in top_factors:
                ...
                try:
                    impact = float(impact)
                    impact_text = f"{impact:+.4f}"
                except Exception:
                    impact_text = str(impact)

                lines.append(f"- {feature}: {impact_text}")
...
"""
    ...
    def generate_compliance_report(
        ...
        return self._call_llm(
            prompt,
            "You are an Enterprise Compliance and AML Officer.",
        )
    ...
    def _simulate_llm_response(self, prompt):
        if "SAR compliance" in prompt or "regulatory" in prompt:
            ...
            return (
                "Based on the transaction data, the device trust score is low (under 40) and VPN detection is enabled, which suggests high probability of unauthorized access."
            )
        else:
            return (
    ...
    def generate_report(self, arg1, arg2=None):
        if isinstance(arg1, dict) and isinstance(arg2, dict):
            transaction_data = arg1
            prediction_output = arg2
            risk_output = arg2
        elif isinstance(arg1, dict):
            transaction_data = arg1
            prediction_output = arg1
            risk_output = arg1
        else:
            transaction_data = {}
            prediction_output = {}
